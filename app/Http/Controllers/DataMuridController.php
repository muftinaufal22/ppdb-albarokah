...
// FIX KRITIS: HAPUS SEMUA USE STATEMENT ALIAS LAMA YANG RUSAK DI SINI
// (e.g., App\Http\Controllers\SPPControllerPPDB\Http\Requests\...)
...
class DataMuridController extends Controller
{
     ...
     * @return Renderable
     */
    public function update(Request $request, $id) // FIX: Menggunakan Request standar
    {
        try {
            DB::beginTransaction();
            // Validator::make digunakan di sini karena method tidak menggunakan FormRequest di signature
    ...
    public function payment($murid)
    {
        try {
            ...
            DB::rollBack();
            Session::flash('error', 'Gagal membuat data pembayaran: ' . $e->getMessage());
        }
    }
}