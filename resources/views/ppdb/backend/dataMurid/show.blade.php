...
<div class="content-wrapper container-xxl p-0">
    ...
    <div class="content-body">
        <div class="row">
            <div class="col-12">
                {{-- Logika alert ini sepertinya terbalik. Seharusnya tampil jika KK == NULL --}}
                {{-- Saya ubah menjadi: {{$murid->berkas->kartu_keluarga == NULL ? '' : 'hidden'}} --}}
                        ...
                        <form action=" {{route('data-murid.update',$murid->id)}} " method="post" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                <div class="col-6">
                                    <ul>
                                        <li>Kartu Keluarga
                                            {{-- PERBAIKAN: Logika badge diubah --}}
                                            @if($murid->berkas->kartu_keluarga != NULL)
                                                <a href="{{asset('storage/images/berkas_murid/' .rawurlencode($murid->berkas->kartu_keluarga))}}" target="_blank" class="badge badge-info">view</a>
                                            @else
                                                <span class="badge badge-light-secondary">Belum ada</span>
                                            @endif
                                        </li>
                                        <li>Akte Kelahiran
                                            {{-- PERBAIKAN: Logika badge diubah dan rawurlencode --}}
                                            @if($murid->berkas->akte_kelahiran != NULL)
                                                <a href="{{asset('storage/images/berkas_murid/' .rawurlencode($murid->berkas->akte_kelahiran))}}" target="_blank" class="badge badge-info">view</a>
                                            @else
                                                <span class="badge badge-light-secondary">Belum ada</span>
                                            @endif
                                        </li>
                                ...
                                <div class="col-6">
                                    <ul>
                                        ...
                                        <li>KTP
                                            {{-- PERBAIKAN: Logika badge diubah dan rawurlencode --}}
                                            @if($murid->berkas->ktp != NULL)
                                                <a href="{{asset('storage/images/berkas_murid/' .rawurlencode($murid->berkas->ktp))}}" target="_blank" class="badge badge-info">view</a>
                                            @else
                                                <span class="badge badge-light-secondary">Belum ada</span>
                                            @endif
                                        </li>
                                        <li>Foto
                                            {{-- PERBAIKAN: Logika badge diubah dan rawurlencode --}}
                                            @if($murid->berkas->foto != NULL)
                                                <a href="{{asset('storage/images/berkas_murid/' .rawurlencode($murid->berkas->foto))}}" target="_blank" class="badge badge-info">view</a>
                                            @else
                                                <span class="badge badge-light-secondary">Belum ada</span>
                                            @endif
                                        </li>
